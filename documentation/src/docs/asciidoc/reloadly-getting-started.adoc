...
Following pre-requisites need to be fulfilled before you can run a fully functional instance of the platform on your local development environment.
The platform has been tested on macOS Big Sur.

1. *Hardware*: Either physical or virtual machines with minimum 8 GB RAM and 4 CPU Cores _(16 GB RAM, 8 CPU Cores recommended)_.
...
Clone the git https://github.com/arunkpatra/reloadly-services[repository] from GitHub into a directory of your choice on your local machine.
...
This is useful, when you want to setup a development environment and run all the microservices on a single local machine.
Kubernetes Deployment support is coming up. See  https://github.com/arunkpatra/reloadly-services/issues/17[this GitHub issue].
...
As noted earlier, you do not need to set up Kafka and MySQL for running integration tests since tests use in-memory Derby database.
...
It does nothing other than adding a service account, and an API key which is needed for inter-microservice communication.
The unencrypted value (for testing only) of the API key is _test-api-key_. Also note the `client_id`.
...
You should follow these steps sequentially. The steps mentioned below run the services as spring boot apps. However, you could run all these services in a Kubernetes cluster as described in <<running-services-on-kubernetes, this section>>. A convenient Helm Chart is provided and is described in <<running-services-on-kubernetes-using-helm, this section.>>
...
|RELOADLY_CLIENT_ID
|The reloadly service account client ID.
Use the value you inserted in the <<seed-data, Seed Data>> section.
|Needed for the platform to work correctly.
Use the value of `client_id` from the final `INSERT` statement in that script. (This is for testing only; should not be used for production)

|RELOADLY_SVC_ACCT_API_KEY
|The reloadly service account API key.
Use the value you inserted in the <<seed-data, Seed Data>> section.
|Needed for the platform to work correctly.
Use the unencrypted value of `api-key` from the final `INSERT` statement in that script. The pre-seeded value is _test-api-key_. (This is for testing only; should not be used for production)
|===
